...
{
//wp_redirect(site_url().'/wp-admin/admin.php?page=bulk_update&emsg=id_title');exit;
...
if($post_image){
	for($im=0;$im<count($post_image);$im++)
	{
		...
		@wp_update_attachment_metadata( $last_postimage_id, $post_attach_arr );
			...
			echo "<br><p class=\"updated below-h2\"><br>CSV uploaded successfully.";
			echo "<br><br>Total of $row_update_counter records updated successfully.</b><br><br></p>";			
		...
		else
		{
			...
			echo '<script>document.error_form_redirect.submit();</script>';
			exit;
			//wp_redirect(site_url().'/wp-admin/admin.php?page=bulk_update&emsg=nomove');exit;
		}
	}else
	{
		$form_url = site_url().'/wp-admin/admin.php';
		...
		echo '<script>document.error_form_redirect.submit();</script>';
		exit;
		//wp_redirect(site_url().'/wp-admin/admin.php?page=bulk_update&emsg=csvonly');exit;
	}
}else
{
	$form_url = site_url().'/wp-admin/admin.php';
	...
	echo '<script>document.error_form_redirect.submit();</script>';
	exit;
	//wp_redirect(site_url().'/wp-admin/admin.php?page=bulk_update&emsg=wrongfile');exit;
}
?>